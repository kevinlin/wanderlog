 ...
 *   3. og:image from place URL
 *   4. Web search by place name
 *
 * Activity type inference:
 *   1. Existing activity_type (preserved)
 *   2. Google Places API types → inferred type
 *   3. Activity name keywords → inferred type
 */
...
async function enrichActivity(activity: Activity): Promise<{ activity: Activity; enriched: boolean; typeEnriched: boolean }> {
  ...
  if (anyChange) {
    ...
    console.log(`     ✅ Enriched (${changes.join(', ')})`);
    return {
      activity: {
        ...activity,
  ...
  if (activity.activity_type) {
    console.log(`     ⏭️  Skipped (type: ${activity.activity_type})`);
  } else {
    console.log('     ⏭️  Skipped (already has data or no results)');
  }
  return { activity, enriched: false, typeEnriched: false };
...
async function main(): Promise<void> {
  ...
  console.log('  - activity_type: Activity type classification\n');
