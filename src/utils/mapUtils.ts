 ...
 */
export const formatCoordinatesForUrl = (coord: Coordinates): string => `${coord.lat.toFixed(6)},${coord.lng.toFixed(6)}`;
...
export const calculateEstimatedTravelTime = (
  ...
  averageSpeedKmh = 60 // Default highway speed
): number => {
 ...
 */
export const generateDirectionsUrl = (origin: Coordinates, destination: Coordinates, waypoints?: ScenicWaypoint[]): string => {
 ...
 */
export const isWithinRadius = (center: Coordinates, point: Coordinates, radiusKm: number): boolean => {
 ...
 */
export const snapToGrid = (coord: Coordinates, gridSizeDegrees = 0.001): Coordinates => ({
