...
- 2026-07-26: Schema and offline rules amended by [requirements_phase-4.md](../phase-4/requirements_phase-4.md): `activities.duration` and `scenic_waypoints.duration` become `duration_minutes` (integer, migrated by parsing the existing text); both tables gain `visited_at` (text, `'YYYY-MM-DD HH:mm'` local to the trip timezone, same rationale as `check_in`/`check_out`). The offline edit-disable rule gains one exemption, the visit-details form.
