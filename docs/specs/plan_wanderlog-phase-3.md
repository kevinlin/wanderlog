...
| M2 - Bounded edits | (just-in-time) | Not started | Scripted edit prompts round-trip on a preview; deletes fire only on explicit request; partial failure reported honestly |
| M3 - Generative creation + programmatic contract | (just-in-time) | Not started | Creation prompt yields a complete rendered trip; Hermes-style curl session (password grant → buffered call) works end-to-end |
